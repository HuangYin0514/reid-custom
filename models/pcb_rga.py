...
class Resnet50_Moudule(nn.Module):
    def __init__(self):
        super(Resnet50_Moudule, self).__init__()
...
class PCB_RGA(nn.Module):
    def __init__(self, num_classes, parts=6, loss='softmax', ** kwargs):
        ...
        # backbone--------------------------------------------------------------------------
        self.resnet50_Moudule = Resnet50_Moudule()
    ...
    def forward(self, x):
        # backbone------------------------------------------------------------------------------------
        resnet_features = self.resnet50_Moudule(x)
...
def pcb_rga(num_classes, parts=6, **kwargs):
    assert parts == 6, "parts not eq 6"
    return PCB_RGA(
